...
# 뉴스 카테고리별 구글뉴스 검색어 (tag, 검색어, 언어, 지역)
NEWS_QUERIES = [
    ("한국", "코스피 OR 반도체 OR 삼성전자 OR SK하이닉스", "ko", "KR"),
    ("미국", "미국증시 OR 엔비디아 OR 연준 OR 나스닥", "ko", "KR"),
    ("전쟁", "중동 분쟁 OR 우크라이나 전쟁 OR 호르무즈 OR 지정학 유가", "ko", "KR"),
]
NEWS_TOTAL = 10                   # 화면에 보여줄 뉴스 총 개수

# 임시 호재/악재 판정 규칙 (3-C에서 AI로 업그레이드 예정)
POS_WORDS = ["수주", "양산", "급증", "상승", "회복", "확대", "호조", "기대", "인하", "순매수", "강세", "신고가"]
NEG_WORDS = ["둔화", "규제", "우려", "하락", "부진", "감소", "긴장", "전쟁", "악화", "리스크", "약세", "쇼크", "충돌"]
...
def classify_impact(title):
    """제목 키워드로 반도체 시장 호재/악재/중립 임시 판정 (3-C에서 AI로 교체)"""
    pos = sum(w in title for w in POS_WORDS)
    neg = sum(w in title for w in NEG_WORDS)
    if pos > neg:
        return "pos"
    if neg > pos:
        return "neg"
...
def get_news():
    """구글뉴스 RSS에서 카테고리별 최신 뉴스 수집"""
    collected = []
    per_cat = max(1, NEWS_TOTAL // len(NEWS_QUERIES) + 1)
    for tag, query, hl, gl in NEWS_QUERIES:
        q = urllib.parse.quote(query)
        url = f"https://news.google.com/rss/search?q={q}&hl={hl}&gl={gl}&ceid={gl}:{hl}"
        try:
            feed = feedparser.parse(url)
            for entry in feed.entries[:per_cat]:
                title = entry.title.split(" - ")[0].strip()  # 끝의 언론사명 제거
                collected.append({
                    "tag": tag,
                    "text": title,
...
def main():
    ...
    payload = {
        ...
        "fx": fx,
        "stocks": stocks,
        "news": get_news(),   # 3-A: 실제 뉴스 (호재/악재는 임시 규칙)
        "summary": "",        # 3-C에서 AI로 채움
    }
